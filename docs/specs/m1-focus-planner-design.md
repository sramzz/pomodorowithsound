...
- **Sync Stub:** a Rust-side sync seam — a no-op `SyncService` trait inside the Domain Core. The schema stays sync-ready (client-generated UUIDs, `updated_at` on every table, no AUTOINCREMENT). Explicitly **no PowerSync JS SDK**: it manages its own SQLite, which would create a second database in the webview and bypass the Core's single write path. ADR for M3 (ecosystem status as of June 2026): three candidate paths — (a) load the PowerSync core extension into the SQLx connection (SQLx 0.9, in alpha, adds compile-time SQLite extension loading, which fits this exactly); (b) the official **PowerSync Tauri SDK** (alpha) — architecturally aligned with us (SQLite on the filesystem, sync connection runs in Rust), but its JS package exposes the full PowerSync API *including writes* to the webview, so we could only adopt its Rust layer without violating the single-write-path rule; (c) the standalone PowerSync Rust SDK — still pre-alpha, SQLx interop only "under investigation". Decision deferred to M3; revalidate maturity then.
